...
for Eap in Eap_values:
    ...
    k1_1_v = k1_1 * v_values[0]  # Use the largest scan rate for plotting
...
# Calculate the area between i1 and i2 for total current
delta_i_total = np.abs(np.array(i1_total_plot) - np.array(i2_total_plot))
a1 = np.trapz(delta_i_total, Eap_plot)

# Calculate the area between k1_1*v and k1_2*v for total current
...
# Plot i1 and i2 for total current
...
plt.fill_between(Eap_plot, k1_1_v_plot, k1_2_v_plot, color='green', alpha=0.5, linewidth=5, label=f'Percent error: {percent_error*100:.1f}%')
