...
# E0 quantities (different number of peaks)
...
percent_errors = np.zeros((len(v_values), len(E0_quantities)))

# Main loop for calculating percent errors for different v and E0 quantities
for j, num_E0 in tqdm(enumerate(E0_quantities), total=len(E0_quantities)):
    for i, v_current in enumerate(v_values):
        results = []
        E0_values = np.linspace(0.5, -0.5, num_E0)  # Generate the required number of E0 values
        ...
        for Eap in Eap_values:
            ...
            i2_total = []
            for v in v_values:
                # Calculate EDLC and total current
            ...
            # Calculate k1_1*v and k1_2*v for total current (use the largest scan rate for plotting)
            ...
            results.append({
                'Eap': Eap,
                'i1_total': i1_total[0],  # Store total current at the largest scan rate
                'i2_total': i2_total[0],  # Store total current at the largest scan rate
                'k1_1_v': k1_1_v,
                'k1_2_v': k1_2_v
            })
        ...
        percent_errors[i, j] = percent_error  # Store the error for heatmap

# Plotting the heatmap
cmap0 = mpl.colors.LinearSegmentedColormap.from_list('red2green', ['green', 'orange', 'red'])
plt.figure(figsize=(12, 8))
plt.contourf(v_values, list(E0_quantities), percent_errors.T, levels=100, cmap=cmap0)

plt.xlabel('Scan Rate (V/s)', fontsize=24)
